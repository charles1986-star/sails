...
router.post('/shops', verifyToken, verifyAdmin, upload.single('image'), async (req, res) => {
  try {
    const { name, description, shop_category_id, contact_name, sku, brand, model_number, color, material, price, status } = req.body;
    if (!name) return res.status(400).json({ msg: 'Name required', type: 'error' });

    const imagePath = req.file ? `/uploads/${req.file.filename}` : null;
    await db.query(
      `INSERT INTO shops (name, description, shop_category_id, contact_name, image_url, sku, brand, model_number, color, material, price, status)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [name, description || null, shop_category_id || null, contact_name || null, imagePath, sku || null, brand || null, model_number || null, color || null, material || null, price || null, status || 'active']
    );
...
router.put('/shops/:id', verifyToken, verifyAdmin, upload.single('image'), async (req, res) => {
  try {
    ...
    await db.query(
      `UPDATE shops 
       SET name = ?, description = ?, shop_category_id = ?, contact_name = ?, image_url = ?, sku = ?, brand = ?, model_number = ?, color = ?, material = ?, price = ?, status = ?
       WHERE id = ?`,
      [name, description || null, shop_category_id || null, contact_name || null, img, sku || null, brand || null, model_number || null, color || null, material || null, price || null, status || 'active', id]
    );
